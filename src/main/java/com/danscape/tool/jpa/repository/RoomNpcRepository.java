...
public interface RoomNpcRepository extends JpaRepository<RoomNpcEntity, RoomNpcCompositeKey> {
	@Query(value="select floor, tile_id, npc_id from room_npcs where floor=:floor and (tile_id%46325 between :x and :x+:w) and (floor(tile_id/46325) between :y and :y+:h)", nativeQuery=true)
