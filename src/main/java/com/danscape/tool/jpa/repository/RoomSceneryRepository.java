...
public interface RoomSceneryRepository extends JpaRepository<RoomSceneryEntity, RoomSceneryCompositeKey> {
	@Query(value="select floor, tile_id, scenery_id from room_scenery where floor=:floor and (tile_id%Config.MAP_ROW_TILE_LENGTH between :x and :x+:w) and (floor(tile_id/Config.MAP_ROW_TILE_LENGTH) between :y and :y+:h)", nativeQuery=true)
