...
public interface RoomGroundTextureRepository extends JpaRepository<RoomGroundTextureEntity, Integer> {
	...
	@Query(value="select floor, tile_id, ground_texture_id from room_ground_textures where floor=:floor and (tile_id%46325 between :x and :x+:w) and (floor(tile_id/46325) between :y and :y+:h)", nativeQuery=true)
